...
from consav.misc import nonlinspace, create_shocks # various functions
...
class DurableConsumptionModelClass(ModelClass):
    ...
    def setup(self):
        ...
        self.not_float_list = [ 'do_2d','T','t','Npsi','Nxi','Nm','Np','Nn','Nx','Na','Nshocks','simN','sim_seed',
                                'do_print','do_print_period','cppthreads','do_marg_u','do_simple_wq']
    ...
    def solve(self,do_assert=True):
        ...
        for t in reversed(range(par.T)):
            ...
            if t == par.T-1:
                ...
                last_period.solve(t,self.sol,par)
            ...
            # ii. all other periods
                ...
                if self.solmethod in ['nvfi']:
                    post_decision.compute_wq(t,self.sol,par)
                elif self.solmethod in ['negm']:
                    post_decision.compute_wq(t,self.sol,par,compute_q=True)
                ...
                if self.solmethod == 'vfi':
                    vfi.solve_keep(t,self.sol,par)
                elif self.solmethod == 'nvfi':                
                    nvfi.solve_keep(t,self.sol,par)
                elif self.solmethod == 'negm':
                    negm.solve_keep(t,self.sol,par)
                ...
                if self.solmethod == 'vfi':
                    vfi.solve_adj(t,self.sol,par)
                elif self.solmethod in ['nvfi','negm']:
                    nvfi.solve_adj(t,self.sol,par)                  
                elif self.solmethod == 'vfi_cpp':
