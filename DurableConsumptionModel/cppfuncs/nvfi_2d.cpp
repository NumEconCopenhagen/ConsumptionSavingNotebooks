...
double ineq_obj_adj_full(unsigned n, const double *choices, double *grad, void *solver_data_in)
{
...
double obj_adj_d1(double d1, void *solver_data_in)
{
...
double obj_adj_d2(double d2, void *solver_data_in)
{
...
EXPORT void solve_adj_full(par_struct *par, sol_struct *sol, sim_struct *sim)
{
    ...
    auto opt = nlopt_create(NLOPT_LD_MMA, 2);
        ...
        // constraints
        nlopt_add_inequality_constraint(opt, ineq_obj_adj_full, solver_data, 1e-8);
...
EXPORT void solve_adj_d1(par_struct *par, sol_struct *sol, sim_struct *sim)
{
    ...
    for(int i_p = 0; i_p < par->Np; i_p++){
        ...
        for(int i_x = 0; i_x < par->Nx; i_x++){
            ...
            d1[index_adj] = golden_section_search(d_low,d_high,par->tol,solver_data,obj_adj_d1); 
            inv_v[index_adj] = -obj_adj_d1(d1[index_adj],solver_data);
...
EXPORT void solve_adj_d2(par_struct *par, sol_struct *sol, sim_struct *sim)
{
    ...
    for(int i_p = 0; i_p < par->Np; i_p++){
        ...
        for(int i_x = 0; i_x < par->Nx; i_x++){
            ...
            d2[index_adj] =  golden_section_search(d_low,d_high,par->tol,solver_data,obj_adj_d2); 
            inv_v[index_adj] = -obj_adj_d2(d2[index_adj],solver_data);
