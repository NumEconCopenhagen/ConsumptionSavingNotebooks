...
double value_of_choice(int t, double c, double d, double p, double x, sol_struct *sol, par_struct *par)
{
...
double obj_adj(unsigned n, const double *choices, double *grad, void *solver_data_in)
{
    ...
    // gradient
    if(grad){
        double forward_d = -value_of_choice(t,c,d+EPS,p,x,sol,par);
        grad[0] = (forward_d - obj)/EPS;
        double forward_c = -value_of_choice(t,c+EPS,d,p,x,sol,par);
    ...
    for(int i_n = 0; i_n < par->Nn; i_n++){
        ...
        for(int i_m = 0; i_m < par->Nm; i_m++){
            ...
            if(par->use_gs_in_vfi){
                ...
                c[index] = golden_section_search(c_low,c_high,par->tol,solver_data,obj_keep_gs); 
